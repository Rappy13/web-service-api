...
// 伺服器端再次檢查Q16是否已生成、且尚未過期
try {
    $checkStmt = $pdo->prepare('SELECT q16_started_at, q16_expires_at FROM customer WHERE id = :id');
    ...
    if ($customer['q16_started_at'] === null) {
        ...
        echo json_encode(['success' => false, 'message' => '此ID的Q16量表尚未開放作答'], JSON_UNESCAPED_UNICODE);
        exit;
    }

    if (new DateTime() > new DateTime($customer['q16_expires_at'])) {
