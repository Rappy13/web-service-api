<?php
/**
 * GET /api/verify_q16.php?id=XXXXXXXXXX
 *
 * 驗證這組ID的Q16量表是否已生成、且尚未過期
 *
 * 成功回傳: { "success": true, "unit_name": "...", "q16_expires_at": "2026-09-07 15:30:00" }
...
try {
    $stmt = $pdo->prepare('SELECT unit_name, q16_started_at, q16_expires_at FROM customer WHERE id = :id');
    $stmt->execute(['id' => $id]);
    $customer = $stmt->fetch();
    ...
    if ($customer['q16_started_at'] === null) {
        ...
        echo json_encode(['success' => false, 'message' => '此ID的Q16量表尚未開放作答', 'expired' => false], JSON_UNESCAPED_UNICODE);
        exit;
    }

    $expiresAt = new DateTime($customer['q16_expires_at']);
    $now = new DateTime();
    ...
    echo json_encode([
        ...
        'q16_expires_at' => $customer['q16_expires_at'],
